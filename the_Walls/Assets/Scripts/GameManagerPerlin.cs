...
public class GameManagerPerlin : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Vector3 cameraPosition = terrain.transform.position + Vector3.up * Mathf.Max(x, y) * 1.3f + new Vector3((x - 1) / 2, 0, 0) -Vector3.forward * x;
    ...
    void FixedUpdate()
    {
        ...
        if (thirdPersonView) {
            ...
            if (Input.gyro.attitude != Quaternion.identity && gyroPointer.transform.up.y > .3f && gyroPointer.transform.forward.z > .7f)
            {
                ...
                if (gyroPointer.transform.up.z > .1f) endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) - myCamera.transform.forward * playerSpeed - endMaterial.transform.position) * Time.fixedDeltaTime);
                if (gyroPointer.transform.up.x < -.2f) pointer.transform.Rotate(Vector3.up);
                if (gyroPointer.transform.up.x > .2f) pointer.transform.Rotate(-Vector3.up);
        ...
        else
        {
            if (Input.gyro.attitude != Quaternion.identity && gyroPointer.transform.up.y > .3f && gyroPointer.transform.forward.z > .7f)
            {
                ...
                if (gyroPointer.transform.up.z < -.1f) endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) + new Vector3(0, 0, playerSpeed) - endMaterial.transform.position) * Time.fixedDeltaTime);
                if (gyroPointer.transform.up.z > .1f) endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) + new Vector3(0, 0, -playerSpeed) - endMaterial.transform.position) * Time.fixedDeltaTime);

                if (gyroPointer.transform.up.x > .1f) endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) + new Vector3(playerSpeed, 0, 0) - endMaterial.transform.position) * Time.fixedDeltaTime);
                if (gyroPointer.transform.up.x < -.1f) endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) + new Vector3(-playerSpeed, 0, 0) - endMaterial.transform.position) * Time.fixedDeltaTime);
