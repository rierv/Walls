    ...
    void FixedUpdate()
    {
        ...
        if (thirdPersonView) {
            ...
            if (Input.gyro.attitude != Quaternion.identity)
            {
                ...
                endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) + myCamera.transform.forward * Mathf.Clamp(Input.gyro.attitude.x * 5, -3, 3) - endMaterial.transform.position) * Time.fixedDeltaTime / 2);
