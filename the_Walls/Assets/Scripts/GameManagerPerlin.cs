    ...
    void FixedUpdate()
    {
        ...
        if (thirdPersonView) {
            ...
            if (Input.gyro.attitude != Quaternion.identity)
            {
                ...
                if(Mathf.Abs(Input.gyro.attitude.y)>.08f) endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) + myCamera.transform.forward * Mathf.Clamp(correctedQuaternion.eulerAngles.y * 50, -2, 2) - endMaterial.transform.position) * Time.fixedDeltaTime);

                if (Mathf.Abs(Input.gyro.attitude.x) > .08f) pointer.transform.Rotate(Vector3.up * Mathf.Clamp(-correctedQuaternion.x*5, -2, 2));
            }
        ...
        {
            if (Input.gyro.attitude != Quaternion.identity)
                endMaterial.transform.Translate((getNodePosition(matrix[xEnd, yEnd]) + new Vector3(Mathf.Clamp(correctedQuaternion.x * 5, -2, 2), 0, Mathf.Clamp(correctedQuaternion.y * 5, -2, 2)) - endMaterial.transform.position) * Time.fixedDeltaTime);
